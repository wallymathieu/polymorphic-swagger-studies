...
public class ApiController implements Api {
                ...
                return commandsHandler.handle(command)
                                .thenApply(result -> result.fold(
                                                len -> new ResponseEntity<>(
                                                                this.mapToOrderModel(repository
        ...
        public CompletableFuture<ResponseEntity<ProductModel>> addProduct(@Valid AddProduct body) {
                ...
                return commandsHandler.handle(command)
                                .thenApply(result -> result.fold(
                                                len -> new ResponseEntity<>(
                                                                this.mapToProductModel(repository
        ...
        public CompletableFuture<ResponseEntity<CustomerModel>> createCustomer(@Valid CreateCustomer createCustomer) {
                ...
                return commandsHandler.handle(command)
                                .thenApply(result -> result.fold(
                                                len -> new ResponseEntity<>(
                                                                this.mapToCustomerModel(repository
        ...
        public CompletableFuture<ResponseEntity<CustomerModel>> getCustomer(BigInteger id) {
                return CompletableFuture
                                .supplyAsync(() -> repository.tryGetCustomer(UUIDUtils.convertFromBigInteger(id))
                                                .map(this::mapToCustomerModel).map(ResponseEntity::ok)
        ...
        public CompletableFuture<ResponseEntity<List<CustomerModel>>> getCustomers() {
                return CompletableFuture.supplyAsync(
                                () -> ResponseEntity.ok(repository.getCustomers().stream().map(this::mapToCustomerModel)
                                                .toList()));
        }
        ...
        public CompletableFuture<ResponseEntity<OrderModel>> getOrder(BigInteger id) {
                return CompletableFuture
                                .supplyAsync(() -> repository.tryGetOrder(UUIDUtils.convertFromBigInteger(id))
                                                .map(this::mapToOrderModel).map(ResponseEntity::ok)
                                                .orElse(ResponseEntity.status(HttpStatus.NOT_FOUND).body(null)));
        }
        ...
        public CompletableFuture<ResponseEntity<List<OrderModel>>> getOrders() {
                return CompletableFuture.supplyAsync(
                                () -> ResponseEntity.ok(
                                                repository.getOrders().stream().map(this::mapToOrderModel).toList()));
        }
        ...
        public CompletableFuture<ResponseEntity<ProductModel>> getProduct(BigInteger id) {
                return CompletableFuture
                                .supplyAsync(() -> repository.tryGetProduct(UUIDUtils.convertFromBigInteger(id))
                                                .map(this::mapToProductModel).map(ResponseEntity::ok)
                                                .orElse(ResponseEntity.status(HttpStatus.NOT_FOUND).body(null)));
        }
        ...
        public CompletableFuture<ResponseEntity<List<ProductModel>>> getProducts() {
                return CompletableFuture.supplyAsync(
                                () -> ResponseEntity.ok(repository.getProducts().stream().map(this::mapToProductModel)
                                                .toList()));
        }
